#!/usr/bin/env python3
"""
Actualiza index.html (dashboard Engagement) leyendo los datos directamente
desde el Google Sheet alimentado por Aleph.

OJO: el dashboard de Engagement (a diferencia de Activation y Reactivation)
usa nombres de campo distintos en su DATA.metrics -- minusculas y abreviados
(rts_com, rts_nc, rts_ctl, subs_com, subs_nc, subs_ctl, win_com, win_nc,
win_ctl, num_incentivos, incentivos_con_comms) en vez de las mayusculas que
trae la planilla (RTS_COMUNICADOS, etc). Este script hace ese mapeo. Tambien
usa date_range.from / date_range.to (no start/end).
...
TAB_Q1 = "Engagement query 1"
TAB_Q2 = "Engagement query 2"
...
# Nombres tal cual vienen de la planilla (headers de Query 1)
SHEET_FIELDS = [
...
# Mapeo -> nombres que espera el JS de ESTE dashboard (index.html de Engagement)
FIELD_MAP = {
    "NUM_INCENTIVOS": "num_incentivos",
    "INCENTIVOS_CON_COMMS": "incentivos_con_comms",
    "RTS_COMUNICADOS": "rts_com",
    "RTS_NO_COMUNICADOS": "rts_nc",
    "RTS_CONTROL": "rts_ctl",
    "SUBS_COMUNICADOS": "subs_com",
    "SUBS_NO_COMUNICADOS": "subs_nc",
    "SUBS_CONTROL": "subs_ctl",
    "WINNERS_COMUNICADOS": "win_com",
    "WINNERS_NO_COMUNICADOS": "win_nc",
    "WINNERS_CONTROL": "win_ctl",
}
...
def main():
    ...
    for (week, country, tag, itype), vals in sorted(agg.items()):
        ...
        # traducir a los nombres que espera el JS de este dashboard
        row.update({FIELD_MAP[k]: vals[k] for k in SHEET_FIELDS})
    ...
    DATA = {
        ...
        "incentive_classification": "Engagement",
    ...
    tot_inc = sum(v.get("num_incentivos", 0) for v in metrics_rows)
    tot_subs_ctl = sum(v.get("subs_ctl", 0) for v in metrics_rows)
    tot_win_ctl = sum(v.get("win_ctl", 0) for v in metrics_rows)
    print(f"Total incentivos: {tot_inc}")
    print(f"Subs control (esperado ~0): {tot_subs_ctl}")
    print(f"Winners control (esperado ~0): {tot_win_ctl}")
